...
$DefaultGamePath = "C:\Program Files (x86)\Steam\steamapps\common\GRID Autosport"
...
Write-Host "EgoNet Revival - GRID Autosport installer"
...
$runningGame = Get-Process -Name "GRIDAutosport", "GRIDAutosport_avx" -ErrorAction SilentlyContinue
if ($runningGame) {
    Write-Host "Closing GRIDAutosport before patching..."
...
foreach ($line in $existingHosts) {
    $trimmed = $line.Trim()
    if ($trimmed -eq "# EgoNet Revival GRIDAutosport") {
        continue
    }
...
$cleanHosts.Add("# EgoNet Revival GRIDAutosport") | Out-Null
...
Patch-Executable -ExecutablePath (Join-Path $GamePath "GRIDAutosport.exe") -RootBytes $rootBytes
Patch-Executable -ExecutablePath (Join-Path $GamePath "GRIDAutosport_avx.exe") -RootBytes $rootBytes

Write-Host "Testing HTTPS health endpoint..."
& curl.exe -fsS --ssl-no-revoke "https://prod.egonet.codemasters.com/health" | Out-Host

Write-Host ""
Write-Host "Done. Open GRIDAutosport and enter RaceNet / Rivals."
